...
- When the battery starts above the target, lets it naturally discharge while
  plugged in until the macOS visible battery percentage reaches the target, then
  keeps charging paused until the battery drops below the hysteresis threshold.
- Shows the visible battery percentage and the raw battery gauge in the menu
  status. The visible percentage is the user-facing target; the raw gauge is
  used only as a safety floor during discharge-to-target.
- Installs a root helper daemon for the low-level SMC write.
- Provides a Launch at Login toggle for the menu bar controller.
- Shows the current app version in the menu and can check GitHub Releases for a
  newer version, then opens the release page for manual download.
- Dims the menu bar icon when charge limiting is not active, and shows a bright
  icon when the charge limit rule is active.
...
- Discharge-to-target has only been validated on one Intel MacBook model so far.
- Update checking is notification-only: the app opens GitHub Releases and the
  user replaces the app manually.
...
- 在菜单栏中应用目标电量策略。
- 如果当前电量高于目标值，会在接入电源时让电池自然回落，直到 macOS 系统显示电量
  达到目标值；达到目标后继续暂停充电，直到电量低于回差阈值再恢复充电。
- 菜单状态会同时显示系统可见电量和底层 raw 电量。用户设置的目标值以系统可见电量为准；
  raw 电量只在主动回落过程中作为低水位安全保护。
- 安装一个 root helper daemon 负责底层 SMC 写入。
- 提供“开机自启 / Launch at Login”菜单开关。
- 在菜单中显示当前版本，并可检查 GitHub Releases 是否有新版本；发现新版时会打开
  Release 页面，由用户手动下载替换。
- 充电限制规则未启用或不可用时，菜单栏图标显示为灰色；规则真正启用时显示为明亮状态。
...
- 主动回落到目标电量目前只在一台 Intel MacBook 上验证过。
- 检查更新目前只做提醒和跳转，不会自动下载或替换应用。
